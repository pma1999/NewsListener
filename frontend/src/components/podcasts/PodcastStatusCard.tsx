  ...
  const renderStatusContent = () => {
    if (isLoading && !statusData) { // Initial load
      return (
        <div className="flex items-center justify-center text-gray-400">
          ...
          <span>Loading status... (ID: {newsDigestId})</span>
        </div>
      );
    }
      ...
      return (
        <div className="p-3 bg-red-900/30 border border-red-700 rounded-md text-red-300">
          <div className="flex items-center mb-1">
            ...
            <span className="font-semibold">Error fetching status for ID: {newsDigestId}</span>
          </div>
          <p className="text-xs">Details: {error?.message || 'Unknown error'}</p>
        </div>
      );
    }

    if (!statusData) { // Should not happen if isLoading/isError are handled, but as a fallback
      return <p className="text-orange-400">Status not available for ID: {newsDigestId}.</p>;
    }

    const { status, audio_url, script_preview, error_message, updated_at } = statusData;
    const lastUpdated = new Date(updated_at).toLocaleTimeString();
    ...
    switch (status) {
      case NewsDigestStatus.PENDING_SCRIPT:
        statusIcon = <Hourglass size={18} className="animate-pulse" />;
    ...
    return (
      <div className={`p-4 border rounded-lg shadow-md ${status === NewsDigestStatus.COMPLETED ? 'border-green-600 bg-green-900/20' : status === NewsDigestStatus.FAILED ? 'border-red-600 bg-red-900/20' : 'border-gray-700 bg-gray-800/50'}`}>
        <div className="flex items-center justify-between mb-3">
          <h3 className={`text-lg font-semibold ${statusColor} flex items-center`}>
            {statusIcon} <span className="ml-2">{statusText}</span>
          </h3>
          <span className="text-xs text-gray-500">ID: {newsDigestId}</span>
        </div>
        
        {script_preview && (
          <div className="mb-2 p-2 bg-gray-700/50 rounded-md">
            <p className="text-xs text-gray-400 font-medium mb-1 flex items-center"><FileText size={14} className="mr-1 flex-shrink-0" /> Script Preview:</p>
            <p className="text-xs text-gray-300 italic truncate">{script_preview}</p>
          </div>
        )}
        ...
        {error_message && status === NewsDigestStatus.FAILED && (
          <div className="mt-2 p-2 bg-red-800/40 rounded-md">
            <p className="text-xs text-red-300 font-semibold">Error Details:</p>
